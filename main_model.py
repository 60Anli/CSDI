...
from dynamic_graph import DynamicGraphEncoder
...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        graph_config = config["model"].get("graph", {})
        self.graph_enabled = graph_config.get("enabled", False)
        self.graph_dim = graph_config.get("graph_dim", 32)
        ...
        if self.graph_enabled:
            self.emb_total_dim += self.graph_dim
        ...
        if self.graph_enabled:
            self.graph_encoder = DynamicGraphEncoder(
                ...
                graph_topk=graph_config.get("graph_topk", 8),
                temperature=graph_config.get("temperature", 0.1),
            )
    ...
    def get_side_info(self, observed_tp, cond_mask, observed_data=None):
        ...
        side_inputs = [time_embed, feature_embed]
        if self.graph_enabled:
            if observed_data is None:
                raise ValueError("observed_data is required when model.graph.enabled is true")
            graph_embed = self.graph_encoder(observed_data, cond_mask)
            graph_embed = graph_embed.unsqueeze(1).expand(-1, L, -1, -1)
            side_inputs.append(graph_embed)

        side_info = torch.cat(side_inputs, dim=-1)  # (B,L,K,*)
...
class CSDI_AQI36(CSDI_PM25):
    def __init__(self, config, device, target_dim=36):
        super(CSDI_AQI36, self).__init__(config, device, target_dim=target_dim)
...
class CSDI_Forecasting(CSDI_base):
    ...
    def get_side_info(self, observed_tp, cond_mask, feature_id=None, observed_data=None):
        B, K, L = cond_mask.shape
